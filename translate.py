...
def get_args():
    ...
    p.add_argument('--tokenize', type=str, action='store', help='tokenize input string (only if type=plain), one of: nltk or simple')
...
def tokenize_nltk(s):
    ...
    for sent in nltk.sent_tokenize(s):
        ...
        alltoks.append(lowercase_first(toks[0]))
        alltoks.extend(toks[1:])
    return ' '.join(alltoks)
